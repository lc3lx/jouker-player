 ...
 *
 * Modes:
 *   stub (default in tests) — in-memory balances for local dev / unit tests
 *   mongo                   — existing Wallet model + ledger transactions
 *
 * All monetary ops run inside a per-user mutex to prevent double-spend races.
 * Same contract as games/poseidon/poseidonWalletAdapter.js.
 */
...
function deductBalanceStub(userId, amount, meta = {}) {
  ...
  row.version += 1;
  return { balance: row.balance, transactionId: crypto.randomUUID(), meta };
}

function creditBalanceStub(userId, amount, meta = {}) {
  const amt = roundMoney(amount);
...
async function deductBalanceMongo(userId, amount, meta = {}) {
  ...
  await withMongoTransaction(async (session) => {
    await ledgerWithdraw({
      ...
      meta: { source: LEDGER_SOURCE, ...meta },
    });
  ...
  await withMongoTransaction(async (session) => {
    if (debit > 0) {
      await ledgerWithdraw({
        ...
        ledgerType: "game_loss",
        meta: { source: LEDGER_SOURCE, ...meta, leg: debitLeg },
      });
    }
    if (credit > 0) {
      await ledgerDeposit({
        ...
        ledgerType: "game_win",
        meta: { source: LEDGER_SOURCE, ...meta, leg: creditLeg },
      });
    }
  });
