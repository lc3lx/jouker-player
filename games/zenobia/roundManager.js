...
/**
 * Round metadata stays in-memory on a short TTL.
 * Bonus sessions are cached in memory and persisted to Mongo in production so
 * buy-bonus / free-spins (and the banked Bonus Box total) survive reconnects
...
const PERSIST_MODE =
  process.env.ZENOBIA_WALLET_MODE ||
...
function getSessionModel() {
  ...
  return require("../../models/zenobiaBonusSessionModel");
}
...
function createRoundHash(roundId, userId, payload) {
  const secret = process.env.ZENOBIA_HMAC_SECRET || "zenobia-dev-hmac-secret";
  return crypto
...
async function persistSession(session) {
  ...
  try {
    ...
    // Don't break gameplay if persistence blips — memory stays the source for
    // this process and the next ensureLoaded can heal from a prior write.
    console.error("[zenobia] bonus session persist failed:", err?.message || err);
  }
}
...
async function deletePersistedSession(userId) {
  ...
  try {
    ...
    console.error("[zenobia] bonus session delete failed:", err?.message || err);
  }
}

/**
 * Hydrate the memory cache from Mongo if needed. Call at the start of any
...
async function ensureLoaded(userId) {
  ...
  try {
    ...
    console.error("[zenobia] bonus session load failed:", err?.message || err);
    return null;
  }
}
...
function createBonusSession(userId, {
  ...
  // Fire-and-forget persist; callers needing durability await touchSession.
...
/** Bonus Box total banked for this session (carries between free spins). */
